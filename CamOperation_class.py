...
#from armor_recognition import *
import armor_recognition as ar
...
class CameraOperation():
    ...
    def Start_grabbing(self,root,panel,Color):
        if False == self.b_start_grabbing and True == self.b_open_device:
            ...
            try:
                ...
                self.h_thread_handle = threading.Thread(target=CameraOperation.Work_thread, args=(self,root,panel,Color))
    ...
    def Work_thread(self,root,panel,color):
        ...
        while True:
            ...
            if 0 == ret:
                if None == buf_cache:
                    buf_cache = (c_ubyte * stOutFrame.stFrameInfo.nFrameLen)()  #以字节格式缓存
            ...
            if PixelType_Gvsp_RGB8_Packed == self.st_frame_info.enPixelType :
                numArray = CameraOperation.Color_numpy(self,buf_cache,self.st_frame_info.nWidth,self.st_frame_info.nHeight,Color)
                mode = "RGB"

            # Mono8直接显示
            elif PixelType_Gvsp_Mono8 == self.st_frame_info.enPixelType :
                numArray = CameraOperation.Mono_numpy(self,buf_cache,self.st_frame_info.nWidth,self.st_frame_info.nHeight,Color)
                mode = "L"
            ...
            # 如果是彩色且非RGB则转为RGB后显示
                ...
                numArray = CameraOperation.Color_numpy(self,img_buff,self.st_frame_info.nWidth,self.st_frame_info.nHeight,Color)
                mode = "RGB"
            ...
            # 如果是黑白且非Mono8则转为Mono8后显示
                ...
                if ret != 0:
                    ...
                    continue
                cdll.msvcrt.memcpy(byref(img_buff), stConvertParam.pDstBuffer, nConvertSize)
                numArray = CameraOperation.Mono_numpy(self,img_buff,self.st_frame_info.nWidth,self.st_frame_info.nHeight,Color)
                mode = "L"

            #合并OpenCV到Tkinter界面中
            current_image = Image.frombuffer(mode, (self.st_frame_info.nWidth,self.st_frame_info.nHeight), numArray.astype('uint8')).resize((800, 600), Image.LANCZOS)
    ...
    def Mono_numpy(self,data,nWidth,nHeight,Color):
        data_ = np.frombuffer(data, count=int(nWidth * nHeight), dtype=np.uint8, offset=0)
        data_mono_arr = data_.reshape(nHeight, nWidth)
        numArray = np.zeros([nHeight, nWidth, 1],"uint8")
        numArray[:, :, 0] = data_mono_arr
        numArray = ar.imageFilter(numArray,Color)#此处灰度图用三通道图像的方法处理，有可能不准确
        return numArray

    def Color_numpy(self,data,nWidth,nHeight,Color):
        data_ = np.frombuffer(data, count=int(nWidth*nHeight*3), dtype=np.uint8, offset=0)
        #np.frombuffer()将字节数据 data 转换为 NumPy 数组，将字节视为 uint8 类型。这意味着每个字节都被视为无符号的 8 位整数（0 到 255）
        #count=int(nWidth*nHeight*3)：表示读取数组中的元素数量，即图像中总像素数乘以 3（每个像素有红、绿、蓝三种颜色通道）
        ...
        numArray = ar.imageFilter(numArray,Color)##
        return numArray
